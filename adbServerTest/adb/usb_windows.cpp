...
ADB_MUTEX_DEFINE(atransport_lock);	//设备列表锁

devicesMap						devices_list;	//已注册的设备列表
...
void usb_cleanup_handle(usb_handle* handle, int only_offline /*= 0*/) {
	if (NULL != handle) {
		...
		if (only_offline){ return; }//不清理interface_name 延迟 find_devices 重新寻找该设备

		//清理后 find_devices 可以立刻重新检测该设备
